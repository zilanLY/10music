/**
 * Netlify Functions — API 入口 (V2 格式)
 *
 * 复用 Cloudflare Pages Functions 的 API 逻辑，适配 Netlify Functions V2。
 * 运行在 Node.js 环境，支持原生 crypto / fetch。
 */

import { Hono } from 'hono'

import { neteaseRequest, createRequestFactory } from '../../functions/api/src/api/request'
import * as NeteaseApi from '../../functions/api/src/api/netease'
import { unblockSong } from '../../functions/api/src/provider/unm'
...
function getReq(c: any) {
  ...
  const realIP = c.req.header('x-real-ip') || c.req.header('x-forwarded-for') || ''
...
app.all('/api/*', async (c) => {
  ...
  try {
    ...
    const result = await neteaseRequest(apiPath, query, {
      crypto: 'weapi',
      cookie,
      realIP: c.req.header('x-forwarded-for') || '',
...
// ── 健康检查 ──────────────────────────────────────────────────────────
app.get('/api/health', (c) => c.json({ status: 'ok', runtime: 'netlify-functions', version: '1.0.0' }))

// Netlify Functions V2 导出 — Hono app.fetch 兼容标准 Request/Response
