/**
 * Music API Worker — Cloudflare Workers 入口
 * 
 * 框架: Hono (轻量、高性能、Workers 原生支持)
 * 加密: Web Crypto API (零 Node.js 依赖)
 * 请求: 原生 fetch
 * 缓存: Cloudflare KV
 */
...
import { neteaseRequest, createRequestFactory } from './api/request'
import * as NeteaseApi from './api/netease'
import { unblockSong } from './provider/unm'
...
// ── 健康检查 ──────────────────────────────────────────────────────────
app.get('/api/health', (c) => c.json({ status: 'ok', runtime: 'cloudflare-worker', version: '1.0.0' }))
